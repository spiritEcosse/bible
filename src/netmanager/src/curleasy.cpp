...
namespace netmanager {
    ...
    CurlEasy::CurlEasy(QUrl &&url, bool progress, bool sslVerify, QObject *parent) noexcept :
    ...
    void CurlEasy::initSaveFile() {
        if(!downloadFile->open(QIODevice::WriteOnly)) {
            ...
            //            m_saveFile->errorString()); shutdownSaveFile();
        }
    }
    ...
    int CurlEasy::staticCurlXferInfoFunction(void *easyPtr,
        ...
        emit transfer->progress(static_cast<qint64>(downloadTotal),
                                static_cast<qint64>(downloadNow),
                                static_cast<qint64>(uploadTotal),
                                static_cast<qint64>(uploadNow));
