...
namespace modules {
    ...
    private:
        ...
        inline auto getActualDataByField(const ColumnIn &columnIn,
                                         const Array &array,
                                         const ColumnC &columnC,
                                         const ValueC &valueC,
                                         Columns... moduleColumns) const {
            return m_db->storage->select(columns(std::forward<Columns>(moduleColumns)...),
        ...
        inline auto getDataByFieldTrue(const Column &column, Columns... moduleColumns) const {
            return m_db->storage->select(columns(std::forward<Columns>(moduleColumns)...), where(c(column) == true));
        }

        template<class ColumnAssign, class ValueAssign, class ColumnC, class Value>
        inline void updateAllC(const ColumnAssign &columnAssign,
