...
namespace modules {
    ...
    GroupModules::GroupModules(const QJsonObject &qJsonModule) : m_language{qJsonModule.value("lng").toString()} {
        m_name = qJsonModule.value("fil").toString();
        m_region = qJsonModule.value("reg").toString();
        cleanName();
    }

    GroupModules::GroupModules(const QString &language, const QString &name, const QString &region, int groupId) :
        m_language{language}, m_name{name}, m_region{region}, m_groupId{groupId} {}

    GroupModules::GroupModules(QString &&language, QString &&name, QString &&region, int groupId) :
        m_language{std::move(language)}, m_name{std::move(name)}, m_region{std::move(region)}, m_groupId{groupId} {}
    ...
    bool GroupModules::operator==(const GroupModules &other) const {
        return m_language == other.m_language && m_name == other.m_name && m_region == other.m_region &&
               m_languageName == other.m_languageName && m_groupId == other.m_groupId;
    }
    ...
    QDebug operator<<(QDebug debug, const GroupModules &groupModules) {
        return debug << groupModules.m_language << groupModules.m_name << groupModules.m_region
                     << groupModules.m_groupId << groupModules.m_id;
    }
#endif
