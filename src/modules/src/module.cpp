...
namespace modules {
    ...
    Module::Module(const QJsonObject &moduleJson) :
        m_name{moduleJson.value("fil").toString()}, m_description{moduleJson.value("des").toString()},
        m_abbreviation{moduleJson.value("abr").toString()}, m_languageShow{moduleJson.value("aln").toString()},
        m_information{moduleJson.value("inf").toString()}, m_comment{moduleJson.value("cmt").toString()},
        m_copyright{moduleJson.value("lic").toString()}, m_hidden{moduleJson.value("hid").toBool()},
    ...
    Module::Module(const QString &name,
        ...
        m_description{std::move(description)}, m_abbreviation{std::move(abbreviation)},
        m_idGroupModules{idGroupModules}, m_size{size}, m_languageShow{std::move(languageShow)},
        m_information{std::move(information)}, m_comment{std::move(comment)}, m_copyright{std::move(copyright)},
        m_update{std::move(update)}, m_hidden{hidden}, m_defaultDownload{defaultDownload}, m_downloaded{downloaded},
        m_selected{selected}, m_active{active}, m_id{id} {}

    bool Module::operator==(const Module &other) const {
        return m_name == other.m_name && m_description == other.m_description &&
               m_abbreviation == other.m_abbreviation && m_size == other.m_size &&
               m_information == other.m_information && m_comment == other.m_comment &&
               m_copyright == other.m_copyright && m_update == other.m_update && m_hidden == other.m_hidden &&
               m_defaultDownload == other.m_defaultDownload && m_languageShow == other.m_languageShow &&
               m_idGroupModules == other.m_idGroupModules && m_downloaded == other.m_downloaded &&
               m_selected == other.m_selected && m_active == other.m_active;
    }
    ...
    void Module::convertSize(const QString &str) {
        ...
        // WARNING: replace on formattedDataSize. This function was introduced in
        // Qt 5.10.
    }
    ...
    QString Module::getFullPathDb() const {
        QString fullPathFile = QDir::currentPath() + "/modules/" + m_name + "/.SQLite3";
