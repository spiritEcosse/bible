...
namespace modules {
    ...
    Registry::Registry(const QByteArray &url, const QByteArray &infoUrl, const short &priority, const bool &test) :
        m_url{std::move(url)}, m_infoUrl{std::move(infoUrl)}, m_priority{std::move(priority)}, m_test{std::move(test)} {
    }
    ...
    bool Registry::operator==(const Registry &other) const {
        return m_url == other.m_url && m_infoUrl == other.m_infoUrl && m_priority == other.m_priority;
    }
