...
namespace modules {
    ...
    Book::Book(int bookNumber,
        ...
        m_numberChapters(numberChapters), m_id(id) {}

    bool Book::operator==(const Book &other) const {
        return m_bookNumber == other.m_bookNumber && m_shortName == other.m_shortName &&
               m_longName == other.m_longName && m_bookColor == other.m_bookColor && m_id == other.m_id;
    }

#ifndef QT_NO_DEBUG_STREAM
    QDebug operator<<(QDebug debug, const Book &book) {
        return debug << book.m_bookNumber << book.m_shortName << book.m_longName << book.m_bookColor
