...
namespace modules {
    ...
    Host::Host(const QJsonObject &json) :
        m_alias{json.value("alias").toString().toUtf8()}, m_path{json.value("path").toString().toUtf8().toBase64()},
        m_priority{json.value("priority").toInt()}, m_weight{json.value("weight").toInt()} {
        if(m_path.isEmpty()) {
    ...
    Host::Host(const QString &alias, const QByteArray &path, int priority, int weight) :
        m_alias{std::move(alias)}, m_path{std::move(path)}, m_priority{std::move(priority)},
    ...
    bool Host::operator==(const Host &other) const {
        return m_alias == other.m_alias && m_path == other.m_path && m_priority == other.m_priority &&
